...
// Account numbers are sensitive and are never sent to the client - every
// API response (list, get, create, update) is shaped through this function,
// so stripping the field here keeps it out of the page's network traffic
// entirely, not just out of what the UI happens to render. updateLoan below
// preserves the value across saves even though the client never has it to
// resubmit.
function rowToLoan(row) {
  const { account, ...rest } = row.data;
...
async function updateLoan(id, fields) {
  ...
  // Account numbers are stripped out of every API response (see
  // rowToLoan), so the edit form never has one to resubmit and a save
  // would otherwise wipe it out on every edit. Preserve whatever is
  // already on file unless this update explicitly sets a new value (e.g.
  // someone typed one into the blank field to record or correct it).
  if (!("account" in data)) {
    const { rows: existing } = await pool.query(
      "SELECT data->>'account' AS account FROM loans WHERE id = $1",
      [id]
    );
    if (existing[0] && existing[0].account != null) {
      data.account = existing[0].account;
    }
  }
  const { rows } = await pool.query(
...
// The most recent updated_at across every loan - bumped by editing a loan
// or recording/deleting a payment. Lets the dashboard show a "last updated"
// date that keeps itself current instead of a hand-edited string in the
// page's HTML.
async function getLastUpdated() {
  await ensureReady();
  const { rows } = await getPool().query(
    "SELECT MAX(updated_at) AS t FROM loans"
  );
  return rows[0].t;
}
